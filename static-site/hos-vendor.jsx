...
function VendorDashboard() {
  ...
  // Pull in any live customer orders that hit this stall
  const liveForStall = (s.liveOrders || []).filter(o => o.vendorId === vendor.id);
  const mergedActive = [...liveForStall, ...queue.filter(o => o.status !== 'collected')];

  React.useEffect(() => { setQueue(VENDOR_QUEUE_SEED); }, [s.vendorScope]);

  const bump = (id) => setQueue(q => q.map(o => o.id === id ? { ...o, status: NEXT_STATUS[o.status] || o.status } : o).filter(o => o.status !== 'collected'));
  const cooking = mergedActive.filter(o => o.status === 'cooking').length;
  const waiting = mergedActive.filter(o => o.status === 'queued').length;
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '22px 28px 60px' }}>
      ...
      {/* AI stall coach */}
      <StallCoach key={vendor.id} vendor={vendor} queue={mergedActive} stats={stats} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 16 }}>
        ...
        <DeskCard style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {mergedActive.length === 0 && <div style={{ padding: 40, textAlign: 'center', fontFamily: HF.b, fontSize: 13, color: HOS.wheat, opacity: 0.5 }}>All caught up — no active orders.</div>}
            {mergedActive.map(o => (
              <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', borderBottom: `1px solid ${HOS.bordS}`, background: o.fresh ? 'rgba(192,98,42,0.06)' : 'transparent', animation: o.fresh ? 'hosSlideIn 0.4s ease' : 'none' }}>
        ...
        <DeskCard style={{ padding: 0, overflow: 'hidden', alignSelf: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', maxHeight: 420, overflowY: 'auto' }}>
            {vendor.menu.map(m => {
              const on = itemAvailable(m.id);
              const stock = itemStock(m.id);
              const low = on && stock <= 6;
              return (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 18px', borderBottom: `1px solid ${HOS.bordS}`, opacity: on ? 1 : 0.5 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      <span style={{ fontFamily: HF.b, fontSize: 13, color: HOS.parch, textDecoration: on ? 'none' : 'line-through' }}>{m.name}</span>
                      {low && <Pill tone="gold">{stock} left</Pill>}
                      {!on && <Pill tone="red">86</Pill>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
                      <span style={{ fontFamily: HF.m, fontSize: 11, color: HOS.gold }}>{money(m.price)}</span>
                      <span style={{ fontFamily: HF.l, fontSize: 9.5, letterSpacing: '0.1em', color: HOS.wheat, opacity: 0.45 }}>· stock {stock}</span>
                      {low && <button onClick={() => actions.restock(m.id, 12)} style={{ fontFamily: HF.l, fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: HOS.ter, background: 'none', border: `1px solid ${HOS.bord}`, borderRadius: 6, padding: '2px 6px', cursor: 'pointer' }}>Restock +12</button>}
                    </div>
                  </div>
                  <button onClick={() => actions.toggleAvailability(m.id)} style={{ width: 44, height: 24, borderRadius: 14, background: on ? HOS.green : HOS.surf2, border: `1px solid ${on ? HOS.green : HOS.bord}`, cursor: 'pointer', position: 'relative', transition: 'all 0.2s', flexShrink: 0 }}>
