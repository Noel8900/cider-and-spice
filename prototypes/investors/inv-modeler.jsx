...
const MODELER_TIERS = [
  { name: 'Community Investor', glyph: '◇', min: 25000,  max: 74999,  note: 'Newsletter · quarterly updates · Cider Club early access' },
  { name: 'Growth Partner',     glyph: '◈', min: 75000,  max: 199999, note: 'Quarterly briefings · Advisory Board observer seat · full model access' },
  { name: 'Founding Investor',  glyph: '◆', min: 200000, max: Infinity, note: 'Signage feature · annual pairing dinner · equity discussion eligible' },
];

const IRR_LO = 0.17, IRR_HI = 0.20;
const MOD_MIN = 25000, MOD_MAX = 500000, MOD_STEP = 5000;

function modFmtUSD(n) { return '$' + Math.round(n).toLocaleString('en-US'); }
function modFmtK(n)  { return '$' + Math.round(n / 1000) + 'K'; }
function modTierFor(amt) {
  for (var i = MODELER_TIERS.length - 1; i >= 0; i--) if (amt >= MODELER_TIERS[i].min) return MODELER_TIERS[i];
  return MODELER_TIERS[0];
}
    ...
    <section id="modeler" style={{ paddingBottom: '5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '0.92fr 1.08fr', gap: '1px', background: 'rgba(232,193,141,0.07)' }}>
        ...
        <div style={{ background: INV.bg, padding: 'clamp(1.75rem, 3vw, 2.5rem)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: INV_F.l, fontSize: '0.5rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: INV.wheat, opacity: 0.3, marginBottom: '1.5rem' }}>
            <span>{modFmtK(MOD_MIN)} min</span><span>{modFmtK(MOD_MAX)}+</span>
          </div>
          ...
          {/* Tier unlocked */}
          <div style={{ marginTop: '2rem', padding: '1.3rem 1.4rem', background: 'rgba(192,98,42,0.08)', borderTop: `2px solid ${INV.ter}`, display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <span style={{ fontFamily: INV_F.d, fontSize: '1.6rem', color: INV.ter, lineHeight: 1, flexShrink: 0 }} aria-hidden="true">{tier.glyph}</span>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontFamily: INV_F.l, fontSize: '0.5rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: INV.wheat, opacity: 0.45, marginBottom: '0.15rem' }}>Unlocks</div>
                <div style={{ fontFamily: INV_F.d, fontSize: '1.35rem', fontWeight: 400, color: INV.parch, lineHeight: 1.05, whiteSpace: 'nowrap' }}>{tier.name}</div>
              </div>
            </div>
            <p style={{ fontFamily: INV_F.b, fontSize: '0.78rem', color: INV.wheat, opacity: 0.55, lineHeight: 1.7, margin: 0 }}>{tier.note}</p>
          </div>
        </div>
      ...
      <div style={{ marginTop: '1.5rem', padding: '1.5rem 2rem', background: INV.surf, borderLeft: `3px solid rgba(192,98,42,0.35)` }}>
        <p style={{ fontFamily: INV_F.b, fontSize: '0.8rem', color: INV.wheat, opacity: 0.48, lineHeight: 1.8, marginBottom: '0.5rem' }}>
          Illustrative model only. Projected value compounds your selected amount at the base-case 17–20% illustrative IRR over the selected hold — it is not a forecast, an offer of securities, or a guarantee of returns. Distributions are modeled to begin after cash-flow breakeven (Mo. 18–20); the Year 1 ramp loss is covered by the working-capital reserve in the $1.505M raise.
        </p>
