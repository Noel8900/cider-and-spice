...
function MenuRow({ item, vendor, qty }) {
  return (
    <div style={{ display: 'flex', gap: 12, padding: '13px 18px', borderBottom: `1px solid ${HOS.bordS}`, alignItems: 'flex-start' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginBottom: 3 }}>
          <span style={{ fontFamily: HF.b, fontSize: 14.5, fontWeight: 600, color: HOS.parch }}>{item.name}</span>
      ...
      {qty > 0 ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, flexShrink: 0, background: HOS.surf, borderRadius: 20, border: `1px solid ${HOS.bordM}` }}>
          ...
          <Stepper onClick={() => actions.addToCart(vendor, item)} sign="+" />
        </div>
      ) : (
