...
const phases = [
  {
    label: 'Phase 0',
    name: 'Proof-First',
    desc: 'Pop-up events, market tables, and vendor tastings validate demand prior to construction risk.',
  },
  {
    label: 'Phase 1',
    name: 'Outdoor / Mobile',
    desc: 'Food truck court, cider & event pad, and ticketed events gather baseline sales data with lower fixed costs.',
  },
  {
    label: 'Phase 2',
    name: 'Indoor Launch',
    desc: 'Permanent and incubator stalls, shared seating, commissary, gift shop, and licensed cider bar open to the public.',
  },
  {
    label: 'Phase 3',
    name: 'Stabilization',
    desc: 'Monthly vendor reviews, event rhythms established, and operational adjustments executed.',
  },
  {
    label: 'Phase 4',
    name: 'Optimization',
    desc: 'Graduation pathways executed, sponsorships renewed, and backfill cycle managed to maintain 80%+ occupancy.',
  },
];
...
export default function IncubatorProgramSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from('.ip-header', { opacity: 0, y: 28, duration: 0.8, ease: 'power3.out',
        scrollTrigger: { trigger: '.ip-header', start: 'top 85%', once: true } });
      gsap.from('.ip-pathway', { opacity: 0, y: 20, duration: 0.6, stagger: 0.1, ease: 'power3.out',
        scrollTrigger: { trigger: '.ip-pathways', start: 'top 80%', once: true } });
      gsap.from('.ip-phase', { opacity: 0, x: -16, duration: 0.5, stagger: 0.08, ease: 'power2.out',
        scrollTrigger: { trigger: '.ip-phases', start: 'top 82%', once: true } });
      gsap.from('.ip-kpi', { opacity: 0, y: 14, duration: 0.45, stagger: 0.07, ease: 'power2.out',
        scrollTrigger: { trigger: '.ip-kpis', start: 'top 85%', once: true } });
    }, ref);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={ref} id="incubator-program"
      ...
      <div style={{ maxWidth: '80rem', margin: '0 auto' }}>
        ...
        {/* Semilla / Mariposa pathways */}
        <div className="ip-pathways"
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))',
            gap: '1.5rem', marginBottom: '4.5rem' }}>

          <div className="ip-pathway" style={{
            padding: '2rem', border: '1px solid rgba(232,193,141,0.12)',
            background: 'rgba(92,74,48,0.25)',
          }}>
            <span style={{ fontFamily: 'var(--font-cormorant), Georgia, serif',
              fontSize: '2.4rem', fontWeight: 300, color: D3.terracotta }}>Semilla</span>
            <p style={{ fontFamily: 'var(--font-josefin), system-ui, sans-serif',
              fontSize: '0.6rem', letterSpacing: '0.22em', textTransform: 'uppercase',
              color: `${D3.wheat}55`, margin: '0.4rem 0 1rem' }}>Early-Stage Track &middot; ~24 Weeks</p>
            <p style={{ fontFamily: 'var(--font-inter), system-ui, sans-serif',
              fontSize: '0.9rem', lineHeight: 1.75, color: `${D3.wheat}80` }}>
          ...
          <div className="ip-pathway" style={{
            padding: '2rem', border: '1px solid rgba(192,98,42,0.25)',
            background: 'rgba(192,98,42,0.07)',
          }}>
            <span style={{ fontFamily: 'var(--font-cormorant), Georgia, serif',
              fontSize: '2.4rem', fontWeight: 300, color: D3.terracotta }}>Mariposa</span>
            <p style={{ fontFamily: 'var(--font-josefin), system-ui, sans-serif',
              fontSize: '0.6rem', letterSpacing: '0.22em', textTransform: 'uppercase',
              color: `${D3.wheat}55`, margin: '0.4rem 0 1rem' }}>Scale Track &middot; Mature Operators</p>
            <p style={{ fontFamily: 'var(--font-inter), system-ui, sans-serif',
              fontSize: '0.9rem', lineHeight: 1.75, color: `${D3.wheat}80` }}>
        ...
        <div style={{ marginBottom: '4.5rem' }}>
          <h3 style={{ fontFamily: 'var(--font-josefin), system-ui, sans-serif',
            ...
            color: D3.terracotta, marginBottom: '1.75rem' }}>
            Phased Launch Strategy
          </h3>
          <div className="ip-phases" style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
            {phases.map((p, i) => (
              <div key={p.label} className="ip-phase" style={{
                display: 'grid', gridTemplateColumns: '120px 1fr',
                padding: '1.25rem 1.5rem', gap: '1.5rem',
                background: i % 2 === 0 ? 'rgba(92,74,48,0.2)' : 'rgba(44,36,22,0.3)',
                borderLeft: `3px solid ${i < 2 ? `${D3.terracotta}40` : i < 4 ? `${D3.terracotta}80` : D3.terracotta}`,
              }}>
                <div>
                  <span style={{ fontFamily: 'var(--font-josefin), system-ui, sans-serif',
                    fontSize: '0.58rem', letterSpacing: '0.2em', textTransform: 'uppercase',
                    color: D3.terracotta, display: 'block' }}>{p.label}</span>
                  <span style={{ fontFamily: 'var(--font-cormorant), Georgia, serif',
                    fontSize: '1.1rem', color: D3.wheat }}>{p.name}</span>
                </div>
                <p style={{ fontFamily: 'var(--font-inter), system-ui, sans-serif',
                  fontSize: '0.88rem', lineHeight: 1.7, color: `${D3.wheat}75`, margin: 0 }}>
                  {p.desc}
                </p>
              </div>
            ))}
        ...
        <div>
          ...
          <div className="ip-kpis" style={{
            display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1px',
            background: 'rgba(232,193,141,0.06)',
          }}>
            {kpis.map(kpi => (
              <div key={kpi.metric} className="ip-kpi" style={{
                padding: '1.5rem', background: D3.walnut,
                borderTop: `1px solid rgba(232,193,141,0.08)`,
              }}>
                <span style={{ fontFamily: 'var(--font-josefin), system-ui, sans-serif',
                  fontSize: '0.65rem', letterSpacing: '0.18em', textTransform: 'uppercase',
                  color: D3.wheat, display: 'block', marginBottom: '0.6rem' }}>
                  {kpi.metric}
                </span>
                <p style={{ fontFamily: 'var(--font-inter), system-ui, sans-serif',
                  fontSize: '0.84rem', lineHeight: 1.65, color: `${D3.wheat}65`, margin: 0 }}>
                  {kpi.detail}
                </p>
              </div>
            ))}
