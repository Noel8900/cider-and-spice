...
// Three CTAs: Apply for a Stall → /vendors | Explore the Incubator → /incubator | Investor Overview → /investors
    ...
    <section ref={ref} style={{
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: '52rem', margin: '0 auto', textAlign: 'center' }}>
        ...
        <div className="vcta-item" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
          ...
          <Link href="/incubator"
            style={{ border: '1px solid rgba(192,98,42,0.45)', color: `${D3.wheat}cc`, padding: '0.95rem 2.5rem', fontFamily: 'var(--font-josefin), system-ui, sans-serif', fontSize: '0.7rem', letterSpacing: '0.25em', textTransform: 'uppercase', textDecoration: 'none', transition: 'border-color 0.3s, color 0.3s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = `rgba(192,98,42,0.8)`; (e.currentTarget as HTMLAnchorElement).style.color = D3.parchment; }}
            onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = 'rgba(192,98,42,0.45)'; (e.currentTarget as HTMLAnchorElement).style.color = `${D3.wheat}cc`; }}>
