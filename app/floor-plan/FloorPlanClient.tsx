...
function ZoomControls({
  ...
  return (
    ...
    >
      {/* Zoom Out */}
      <button style={btnBase} onMouseEnter={hover} onMouseLeave={leave}
        onClick={onZoomOut} disabled={scale <= MIN_SCALE}
        aria-label="Zoom out"
        title="Zoom out"
      >
      ...
      {/* Scale readout */}
      <div
        className="font-label text-[8px] tracking-[0.12em] uppercase px-3"
        style={{ color: 'rgba(212,168,75,0.50)', minWidth: 42, textAlign: 'center', lineHeight: '32px' }}
      >
      ...
      {/* Zoom In */}
      <button style={btnBase} onMouseEnter={hover} onMouseLeave={leave}
        onClick={onZoomIn} disabled={scale >= MAX_SCALE}
        aria-label="Zoom in"
        title="Zoom in"
      >
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
export default function FloorPlanClient() {
  ...
  useEffect(() => {
    ...
    setIsMobile(mobile);
    if (mobile) setTransform(DEFAULT_MOBILE);
  }, []);

  // ── ScrollTrigger-gated cinematic entrance
  // Fires once when the SVG container scrolls into view (>= 20% visible).
  // Falls back to immediate play if already in viewport on load.
  useEffect(() => {
    if (!svgRef.current || !containerRef.current) return;

    // Set all animated elements invisible before timeline runs
    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.create({
        ...
        // If container is already past the trigger on page load, play immediately
        onEnterBack: () => {},
      });

      // If already visible when component mounts (page loaded directly to floor plan)
  ...
  // ── Zoom helpers
  const STEP = 0.25;
  const zoomIn  = () => setTransform(t => ({ ...t, scale: Math.min(t.scale + STEP, MAX_SCALE) }));
  const zoomOut = () => setTransform(t => ({ ...t, scale: Math.max(t.scale - STEP, MIN_SCALE) }));
  const resetView = () => setTransform(isMobile ? DEFAULT_MOBILE : DEFAULT_DESKTOP);
  // Fit: scale down slightly to show full plan with a touch of breathing room
  const fitView = () => setTransform({ scale: 0.92, x: 0, y: 0 });
  ...
  function onTouchMove(e: React.TouchEvent) {
    ...
    if (e.touches.length === 1 && isPanning.current) {
      setTransform(t => ({ ...t, x: panStart.current.tx + e.touches[0].clientX - panStart.current.x, y: panStart.current.ty + e.touches[0].clientY - panStart.current.y }));
    } else if (e.touches.length === 2 && pinchDist.current !== null) {
      const ratio = getTouchDist(e.touches) / pinchDist.current;
      setTransform(t => ({ ...t, scale: Math.min(Math.max(pinchScale.current * ratio, MIN_SCALE), MAX_SCALE) }));
    }
  }
  ...
  function handleKeyDown(stall: Stall, e: React.KeyboardEvent) {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setActiveStall(prev => prev?.id === stall.id ? null : stall); }
  }
  ...
  return (
    <main className="min-h-screen text-cream" style={{ background: '#100E0A' }}>
      ...
      {/* ── Canvas toolbar: zoom controls (both mobile & desktop) ─────────── */}
      <div className="px-4 pt-5 pb-1">
        <div className="mx-auto max-w-5xl flex items-center justify-between gap-4">
          {/* Mobile hint */}
          <span className="font-label text-[8px] tracking-[0.22em] uppercase sm:hidden" style={{ color: 'rgba(232,211,165,0.20)' }}>
            Pinch to zoom · drag to explore
          </span>
          {/* Desktop label */}
          <span className="font-label text-[8px] tracking-[0.22em] uppercase hidden sm:block" style={{ color: 'rgba(232,211,165,0.20)' }}>
            Interactive Plan · Drag to Pan
          </span>
          {/* Controls */}
          <ZoomControls
